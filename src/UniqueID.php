...
/**
 * @desc 基于雪花算法进行改造的全局唯一ID变种算法(超出了JS的最大整形数----JavaScript支持的最大整型就是53位)
 * 如要支持前端JS，可以参考 https://www.liaoxuefeng.com/article/1280526512029729，将机器位和单位时间粒度调大一些
 * @example
 * 
 * use Ltxiong\AlgTool\UniqueID;
 * $work = new UniqueID(2, 100);
 * for($i = 0; $i < 100; $i++) {
...
{
    /**
     * 总计64位，固定高位1位，其中，41位的毫秒数，7位机器占位符，2位IDC数据中心标识，13位毫秒内自增数
     * 
     * 64位的整数，用于设置时间戳基数(毫秒), 此值越大, 生成的ID越小
     * 固定一个小于当前时间的毫秒数 2020/12/01 00:00:00
     */
    const T_WEPOCH =  1606752000000;

    /**
     * 机器标识占的位数  同一个数据中心(IDC)最多可部128台
     */
    const WORKER_ID_BITS = 7;
    ...
    /**
     * 毫秒内自增数点的位数，同一毫秒内最多可产生 8192个不重复的号
     */
